...
def data():
    ...
    j = len(benscan[0])
    data = alldata[:, 1:j]
    #data = PCA(0.99).fit_transform(testdata)
    benlabel = alldata[:, 0]
...
def gen_model(optimizer, n):
    ...
    inputs = Input((n,))
    l1 = Dense(input_dim = n , units = 128, kernel_initializer=initializers.glorot_uniform())(inputs)
...
def dis_model(optimizer, n):
    inputs = Input((n,))
    l1 = Dense(input_dim = n, units = 1024, kernel_initializer=initializers.glorot_uniform())(inputs)
    l1 = LeakyReLU(0.1)(l1)
    l1 = Dropout(0.1)(l1)
...
fp = open('NEW_UNSW_Mem_Train.log', 'w+')
...
d, g, gn = train(32, trainddos, trainddoslbl, n)
...
fp = open('NEWUNSW_Mem_Test.log', 'w+')
