...
def Load_Data():
    ...
    bendata = PCA(0.9999).fit_transform(bendata)
...
def gen_model(optimizer, n):
    ...
    inputs = Input((n,))
    l1 = Dense(input_dim = n , units = 32, kernel_initializer=initializers.glorot_uniform())(inputs)
...
def optimz():
    ...
    st = time.time()
    acc = generate_result(d, g, gn, testddos, testddoslbl, n)
    et = time.time()
    ett = et - st
    mf = psutil.Process(os.getpid())
    mf = mp.memory_info().rss
    mtm = mf - mi
    return mtm, ett, acc[0], acc[1]

mtm, ett, los, ac = optimz()

print(mtm, ett, los, ac)
